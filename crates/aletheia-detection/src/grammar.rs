                            ...
                            if let Some(book_entry) = lookup_canonical(canonical) {
                                    ...
                                    parse_tail(book_entry, alias.clone(), &tokens, i + 1)
                                {
                                    if !auto_unambiguous {
                                        ...
                                        reference.disambiguation_options = valid.to_vec();
...
fn dedupe(mut refs: Vec<ParsedReference>) -> Vec<ParsedReference> {
    refs.sort_by(|a, b| {
        (a.book, a.chapter, a.verse_start, a.verse_end, a.explicit_verse)
            .cmp(&(b.book, b.chapter, b.verse_start, b.verse_end, b.explicit_verse))
