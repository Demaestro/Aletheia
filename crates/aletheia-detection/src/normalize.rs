...
impl TranscriptNormalizer {
    ...
    pub fn normalize(&self, raw: &str) -> NormalizedText {
        ...
        // Step 4: repair common low-latency STT phonetic misses before
        // number folding. This keeps the realtime command lane usable for
        // Nigerian/accented speech where tiny Whisper often outputs
        // "Jainesi Chapatzu" for "Genesis chapter two".
        let words = rewrite_common_stt_mishearings(&words);

        // Step 5: merge consecutive spelled numbers into digits.
        let words = merge_spelled_numbers(&words);

        // Step 6: rewrite ordinal prefixes ("first", "1st", "i") immediately
...
fn rewrite_common_stt_mishearings(words: &[String]) -> Vec<String> {
    let mut out = Vec::with_capacity(words.len() + 2);
    for word in words {
        match word.as_str() {
            "jainesi" | "jainesis" | "jenesi" | "jenesis" | "jenisi" | "jenisis" | "genesie"
            | "genesy" => out.push("genesis".to_string()),
            "mathew" | "mattue" | "matu" | "matiu" | "mathiew" | "mathiu" => {
                out.push("matthew".to_string())
            }
            "ezikel" | "eziekiel" | "ezekia" | "ezeqiel" | "ezekel" | "ezekyel" => {
                out.push("ezekiel".to_string())
            }
            "levitrikos" | "levitricos" | "levitico" | "levitikus" => {
                out.push("leviticus".to_string())
            }
            "deutronomy" | "dutronomy" | "deutoronomy" | "deuteronome" => {
                out.push("deuteronomy".to_string())
            }
            "habbakuk" | "habakuk" | "habakkuk" => out.push("habakkuk".to_string()),
            "naham" | "nayhum" => out.push("nahum".to_string()),
            "chapatzu" | "chapatu" | "chaptu" | "chaptwo" | "chaptertwo" => {
                out.push("chapter".to_string());
                out.push("two".to_string());
            }
            "chapta" | "chaptah" | "chappa" | "shapter" => out.push("chapter".to_string()),
            "vass" | "vas" | "vs" => out.push("verse".to_string()),
...
fn read_english_int(words: &[String], start: usize) -> Option<(u32, usize)> {
    ...
    if let Some(n) = small_number(&words[start])
        && words.get(start + 1).map(|w| w.as_str()) == Some("hundred")
    {
        total = n * 100;
        consumed = 2;
        matched_any = true;
...
mod tests {
    ...
    fn spells_numbers_to_digits() {
        ...
        assert_eq!(norm("psalm one hundred and nineteen"), "psalm 119");
    ...
    #[test]
    fn rewrites_low_latency_accent_mishears() {
        assert_eq!(norm("Jainesi Chapatzu"), "genesis chapter 2");
        assert_eq!(
            norm("Levitrikos chapter two vass four"),
            "leviticus chapter 2 verse 4"
        );
        assert_eq!(
            norm("Mathew chapter three verse four"),
            "matthew chapter 3 verse 4"
        );
        assert_eq!(
            norm("Eziekiel chapter one verse two"),
