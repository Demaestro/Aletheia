...
fn is_multilingual_model(path: &Path) -> bool {
    ...
    // English-only models have ".en" before the extension: ggml-base.en.bin,
...
impl OfflineSttAdapter {
    ...
    pub fn transcribe(
        ...
        // Scripture-domain vocabulary prompt: biases Whisper's token distribution
        // toward Bible book names without any model re-training. This prevents the
        // most common mis-transcriptions observed in West African church environments:
        //   "Romans"      → "Romance" / "Romanes"
        //   "Philippians" → "Filipino ins"
        //   "Psalms"      → "Psalms" (usually ok) but "Deuteronomy" needs the hint
        //   "Isaiah"      → "Izaya" (Nigerian accent) → still normalised downstream
        // The prompt is intentionally short so it fits within Whisper's 224-token
        // context window without displacing any real speech.
        params.set_initial_prompt(
            "Scripture: Genesis, Exodus, Leviticus, Numbers, Deuteronomy, Joshua, \
             Judges, Ruth, Samuel, Kings, Chronicles, Ezra, Nehemiah, Esther, Job, \
             Psalms, Proverbs, Ecclesiastes, Isaiah, Jeremiah, Ezekiel, Daniel, \
             Hosea, Joel, Amos, Obadiah, Jonah, Micah, Nahum, Habakkuk, Zephaniah, \
             Haggai, Zechariah, Malachi, Matthew, Mark, Luke, John, Acts, Romans, \
             Corinthians, Galatians, Ephesians, Philippians, Colossians, \
             Thessalonians, Timothy, Titus, Philemon, Hebrews, James, Peter, \
             Revelation. Chapter, verse, the Lord said, thus says the Lord.",
        );
        ...
        // (ggml-base.en.bin, ggml-small.en.bin, etc.) do NOT have a
