...
use aletheia_detection::{AccuracyFixture, ReferenceKeywordDetector, evaluate_accuracy_fixtures};
...
pub fn evaluate_accuracy_fixtures_cmd(
    state: &DesktopState,
    ...
    let mut steps: Vec<LocalRehearsalStepDto> = fixtures
        .iter()
        .map(|fixture| {
            ...
            let (step_state, step_detail) = if fixture.expected_reference.is_some() {
                if single.true_positives > 0 {
                    (
                        "healthy",
                        format!(
                            "Detected expected reference in \"{}\"",
                            truncate(fixture.text, 60)
                        ),
                    )
                    ...
                    (
                        "degraded",
                        format!("No detection for \"{}\"", truncate(fixture.text, 60)),
                    )
                }
            } else {
                ...
                if single.false_positives > 0 {
                    (
                        "degraded",
                        format!("False positive for \"{}\"", truncate(fixture.text, 60)),
                    )
                } else {
                    (
                        "healthy",
                        format!(
                            "Correctly suppressed false positive for \"{}\"",
                            truncate(fixture.text, 60)
                        ),
                    )
                }
...
fn rehearsal_fixtures() -> Vec<AccuracyFixture> {
    vec![
        ...
        AccuracyFixture {
            id: "english-psalm23",
            language: "English",
            text: "Turn to Psalm 23 verse 4.",
            expected_reference: Some("Psalm 23:4"),
        },
        AccuracyFixture {
            id: "negative-prayer",
            language: "English",
        ...
        AccuracyFixture {
            id: "negative-generic",
            language: "English",
            text: "The pastor will speak now.",
            expected_reference: None,
        },
    ]
}
