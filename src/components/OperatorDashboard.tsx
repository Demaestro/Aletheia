    ...
    <section className="space-y-7">
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_360px]">
        ...
        <div className="rounded-[6px] border border-white/5 bg-white/5 p-4">
          ...
          <div className="mt-4 space-y-3">
            <StatusLine icon={AudioLines} label="Pulpit mic" detail="Focusrite USB, -14 dB peak" tone="healthy" />
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_360px]">
        <div className="rounded-[6px] border border-white/5 bg-white/5 p-4">
          <div className="flex items-start justify-between gap-4">
            ...
            <StatusPill tone="neutral" label={aiDetection.mode} />
          </div>
          <div className="mt-4 grid gap-3 md:grid-cols-3">
            {aiDetection.adapters.map((adapter) => (
              <div key={adapter.id} className="rounded-[6px] border border-white/5 bg-paper p-3">
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_360px]">
        <div className="rounded-[6px] border border-white/5 bg-white/5 p-4">
          <div className="flex items-start justify-between gap-4">
            ...
            <StatusPill tone="healthy" label={`${aiDetection.supportedLanguages.length} packs`} />
          </div>
          <div className="mt-4 grid gap-3 md:grid-cols-3">
            {aiDetection.supportedLanguages.slice(0, 9).map((language) => (
              <div key={language.code} className="rounded-[6px] border border-white/5 bg-paper p-3">
                <div className="flex items-center justify-between gap-3">
      ...
      <div className="rounded-[6px] border border-white/5 bg-white/5 p-4">
        <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
          ...
          <StatusPill tone="healthy" label={`${aiDetection.accuracyTarget.autoPreviewThreshold}% preview threshold`} />
        </div>
        <div className="mt-4 grid gap-3 md:grid-cols-2">
          {aiDetection.accuracyTarget.strategy.slice(0, 4).map((item) => (
            <p key={item} className="rounded-[6px] border border-white/5 bg-paper px-3 py-2 text-xs leading-5 text-muted">
              {item}
            </p>
          ))}
        </div>
        ...
        <div className="rounded-[6px] border border-white/5 bg-white/5 p-4">
          ...
          <div className="mt-4 space-y-3">
            {integrations.slice(0, 4).map((integration) => (
              <div key={integration.id} className="flex items-center justify-between gap-3 border-b border-white/5 pb-3 last:border-0 last:pb-0">
                <div>
...
function StatusLine({
  ...
  return (
    <div className="flex items-start gap-3">
      <Icon className="mt-0.5 h-4 w-4 text-muted" aria-hidden="true" />
